...
import co.retaila.athena.common.entities.QScope;
...
@NoArgsConstructor
@AllArgsConstructor
public class SearchScopesQuery extends BaseQuery {
    ...
    protected Predicate getPredicate(BooleanExpression expression) {
        if (StringUtils.isNotBlank(name)) {
            expression = expression.and(QScope.scope.name.contains(name));
        }

        if (StringUtils.isNotBlank(domain)) {
            expression = expression.and(QScope.scope.domain.name.contains(domain));
        }
